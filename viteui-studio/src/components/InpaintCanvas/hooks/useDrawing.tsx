...
export function useDrawing({
    ...
    // Always wait for the "load" event when inputImage changes so we use the new image's dimensions
    // (after e.g. uncommit+reject the img can briefly still have old dimensions when this effect runs).
    useEffect(() => {
    ...
    const EMPTY_BOUNDS: CanvasBounds = { mask: null, padding: null };
    const [bounds, setBounds] = useState<CanvasBounds>(EMPTY_BOUNDS);
    const lastBoundsRef = useRef<CanvasBounds | null>(null);

    const rectsEqual = (a: Bounds | null, b: Bounds | null): boolean =>
        a === b || (!!a && !!b && a.x === b.x && a.y === b.y && a.width === b.width && a.height === b.height);

    const applyBounds = useCallback((mask: Bounds | null, padding: Bounds | null) => {
        const next: CanvasBounds = { mask, padding };
        const prev = lastBoundsRef.current;
        if (prev && rectsEqual(prev.mask, mask) && rectsEqual(prev.padding, padding)) return;
    ...
    const getMaskSnapshot = useCallback(() => {
        ...
        const region = bounds.padding ?? bounds.mask ?? getMaskBounds();
        if (!region) {
            console.error("No bounds found");
            return null;
        }
        return drawSnapshot(region);
    }, [bounds.padding, bounds.mask, getMaskBounds, maskCanvasRef, inpaintFullRes]);

    const calculateBounds = useCallback((): { mask: Bounds | null; padding: Bounds | null } => {
        if (!inpaintFullRes) return { mask: null, padding: null };
        const mask = getMaskBounds();
        if (!mask) return { mask: null, padding: null };
        const canvas = maskCanvasRef.current;
        if (!canvas) return { mask, padding: null };

        const blurPadding = maskBlur * 2;
        const expandedX = Math.max(0, mask.x - blurPadding);
        const expandedY = Math.max(0, mask.y - blurPadding);
        const expandedRight = Math.min(canvas.width, mask.x + mask.width + blurPadding);
        const expandedBottom = Math.min(canvas.height, mask.y + mask.height + blurPadding);
        const expanded = { x: expandedX, y: expandedY, width: expandedRight - expandedX, height: expandedBottom - expandedY };

        const paddedX = Math.max(0, expanded.x - inpaintFullResPadding);
        const paddedY = Math.max(0, expanded.y - inpaintFullResPadding);
        const paddedRight = Math.min(canvas.width, expanded.x + expanded.width + inpaintFullResPadding);
        const paddedBottom = Math.min(canvas.height, expanded.y + expanded.height + inpaintFullResPadding);
        ...
        return {
            mask,
            padding: { x: focusX, y: focusY, width: focusedWidth, height: focusedHeight },
        };
    ...
    useEffect(() => {
        const updateBounds = () => {
            const { mask: m, padding: p } = calculateBounds();
            applyBounds(m, p);
        };
