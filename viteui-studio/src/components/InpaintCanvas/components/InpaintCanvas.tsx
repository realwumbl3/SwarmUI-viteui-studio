...
const InpaintCanvas = ({
    ...
    onCloseTimelapse,
    isActive,
    isViewingTimelinePreview = false,
    maskCanvasFocused = false,
}: {
    workspaceId: string;
    currentImage?: string | null
    previewImage?: string | null
    onClearPreview?: () => void
    isViewingTimelinePreview?: boolean
    ...
    onRegisterBoundsProvider?: (provider: (() => CanvasBounds) | null) => void
    ...
    const [isProgrammaticAction, setIsProgrammaticAction] = useState(false);
    ...
    useEffect(() => {
        if (!isActive) {
            ...
            if (document.pointerLockElement === panTargetRef.current) {
    ...
    const handleResetZoom = useCallback(() => {
        ...
        // Reset zoom does not implicitly enable follow-mode; user explicitly toggles fit-to-screen.
        setFitToScreen(false);
    }, [calculateCenterOffset, setFitToScreen, setPanOffset, setZoom]);

    // Low-level helper that (re)computes a fitted zoom/pan for the current image
    // without changing the fitToScreen flag. This is used whenever follow-mode
    // is already enabled and we need to re-apply the fit (resize, image change).
    const applyFitToScreen = useCallback(() => {
        if (!canvasRef.current || !imageRef.current) return;
        const scale = calculateFitToScreenScale();
        setZoom(scale);
        setPanOffset(calculateCenterOffset());
    }, [canvasRef, imageRef, calculateFitToScreenScale, calculateCenterOffset, setZoom, setPanOffset]);

    // User-facing toggle for fit-to-screen:
    // - If currently off, compute a fitted view and enable follow-mode.
    // - If currently on, simply disable follow-mode (keep current zoom/pan).
    const handleFitToScreen = useCallback(() => {
        if (fitToScreen) {
            setFitToScreen(false);
            return;
        }
        setIsProgrammaticAction(true);
        applyFitToScreen();
        setFitToScreen(true);
    }, [applyFitToScreen, fitToScreen, setFitToScreen]);

    // Handle window resize to maintain fit to screen while follow-mode is enabled
    useEffect(() => {
    ...
    }, [fitToScreen, applyFitToScreen]);

    // Handle image load to trigger fit-to-screen if enabled
    ...
    useEffect(() => {
        if (!onRegisterBoundsProvider) return;
        const provider = () => drawing.bounds;
        onRegisterBoundsProvider(provider);
        return () => onRegisterBoundsProvider(null);
    }, [drawing.bounds, onRegisterBoundsProvider]);
    ...
    }, [isActive, handleDocumentMouseUp, handleDocumentMouseDown]);
    ...
    useEffect(() => {
        ...
        if (!panElement || !canvasElement || (!displayImage && !inputImage && !livePreview) || !isActive) return;
        ...
        const handleWheelEvent = (e: WheelEvent): void => {
            ...
            setIsProgrammaticAction(false); // Manual action
    ...
    }, [panOffset, displayImage, inputImage, livePreview, isActive]);
    ...
    return (
        <main className="studio-canvas relative flex flex-col min-h-0 flex-1 overflow-hidden">
            ...
            {/* Video Overlay */}
            {timelapseVideoUrl && (
                <div className="absolute inset-4 z-30 flex items-center justify-center animate-in fade-in duration-200 pointer-events-none">
                    <div className="relative w-full h-full flex flex-col pointer-events-auto shadow-2xl rounded-lg overflow-hidden ring-1 ring-white/10">
                        <VideoPlayer
                            src={timelapseVideoUrl}
                            onClose={onCloseTimelapse}
