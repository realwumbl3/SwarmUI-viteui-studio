...
const Workspace = ({ workspaceId, isActive }: {
    ...
    const [upscaleDialog, setUpscaleDialog] = useState({
        ...
        upscaleTaskId: null as string | null,
    });
    ...
    const playCling = useCallback(() => {
        const audio = new Audio(clingSound);
        audio.volume = 0.5;
    ...
    const loadWorkspaceGenerations = useCallback(async (options?: { bumpCanvasRefreshKey?: boolean; newCommitPath?: string }): Promise<void> => {
        ...
        try {
            ...
            // Prefer explicit newCommitPath (e.g. from refresh response) so canvas shows the new commit even if list order is stale
            const canvasImageUrl = options?.newCommitPath
                ? `${API_BASE_URL}/api/workspaces/${encodeURIComponent(workspaceId)}/${options.newCommitPath}`
                : latestCommit
                    ? getGenerationImageUrl(latestCommit, "full")
                    : null;

            if (canvasImageUrl) {
                if (options?.bumpCanvasRefreshKey) {
                    // Single update so canvas currentImage and refresh key stay in sync (avoids React batching overwriting the new image)
                    updateWorkspaceState((prev) => ({
                        ...prev,
                        canvas: {
                            ...prev.canvas,
                            currentImage: canvasImageUrl,
    ...
    }, [getGenerationImageUrl, setCanvasState, setTimelineState, updateWorkspaceState, workspaceId]);

    const handleTaskComplete = useCallback(
        (completedTaskId: string, data: ProgressData) => {
            const isUpscale = upscaleDialog.upscaleTaskId === completedTaskId;
            ...
            if (data.status === "failed" || data.status === "interrupted") {
                if (isUpscale) {
                    setUpscaleDialog((prev) => ({
                        ...
                        loading: false,
                        upscaleTaskId: null,
                        error: data.status === "failed" && data.error ? String(data.error) : "Upscale failed.",
                    }));
                } else {
                    if (data.status === "failed" && data.error) {
                        alert(`Generation failed: ${data.error}`);
                    }
                    // Interrupted: clear loading/taskId only so restart effect can run (it needs pendingRestart to stay true).
                    // Failed: full reset.
                    if (data.status === "interrupted") {
                        setGenerationState({ loading: false, currentTaskId: null });
                        sessionStorage.removeItem("currentTaskId");
                    } else {
                ...
                // Only refresh after backend has saved (task queue sends this after save)
            ...
            // Ignore engine's early "completed" (no status) so we wait for post-save completion
        },
        [loadWorkspaceGenerations, playCling, resetGenerationState, upscaleDialog.upscaleTaskId]
    );

    const activeTaskId = generation.currentTaskId || upscaleDialog.upscaleTaskId;
    const { progress: progressData, livePreview } = useWebSocketProgress(
        activeTaskId,
        handleTaskComplete
    );
    ...
    const handleRegisterBoundsProvider = useCallback((provider: (() => CanvasBounds) | null) => {
        boundsProviderRef.current = provider;
    }, []);
    ...
    const handleBeforeGenerate = useCallback(() => {
        ...
        const boundsProvider = boundsProviderRef.current;
        if (!provider) {
            console.error("No mask snapshot provider found");
            return;
        }
        const snapshot = provider();
        const nextMode: Partial<typeof mode> = { inpaintMaskSnapshot: snapshot ?? null };
        nextMode.generationBounds = boundsProvider ? boundsProvider() : null;
    ...
    const handleRefreshCanvas = async (): Promise<void> => {
        ...
        if (workspaceInfo && workspaceInfo.path) {
            try {
                const result = await api.refreshGenerationFromSource(workspaceId, workspaceInfo.path);
                const newCommitPath = result?.path;
                // Use new commit path from response so canvas always shows the new commit; single update avoids batching overwriting it
                await loadWorkspaceGenerations({
                    bumpCanvasRefreshKey: true,
                    newCommitPath: newCommitPath ?? undefined,
                });
                // In edit mode the canvas uses generation.inputImage first; update it so the new commit is shown
                if (newCommitPath) {
                    const newImageUrl = `${API_BASE_URL}/api/workspaces/${encodeURIComponent(workspaceId)}/${newCommitPath}`;
                    setGenerationState({ inputImage: newImageUrl });
                }
            } catch (error) {
    ...
    const loadInitialData = useCallback(async (): Promise<void> => {
        try {
            ...
            if (currentModelTitle && currentModelTitle !== "None") {
                const currentModel = modelsData.find((model) =>
                    model.model_name === currentModelTitle ||
                    model.title === currentModelTitle
                );
                if (currentModel) {
                    setGenerationState({ selectedModel: currentModel.model_name });
                }
            } else {
                setGenerationState({ selectedModel: "None" });
            }

            // Clip Skip is now managed by WorkspaceContext
    ...
    // Keep inpaint/img2img inputImage in sync with canvas when canvas image changes (e.g. after uncommit+reject, or on restore).
    // This ensures the displayed image and mask/drawing dimensions match so cursor coordinates are correct.
    // When workspaceChangingRef is true we only skip if inputImage is already set, so restore-after-load still populates inputImage.
    useEffect(() => {
        if (!workspaceId) return;
        if (workspaceChangingRef.current && generation.inputImage != null) return;
        if (mode.generationMode !== "inpaint" && mode.generationMode !== "img2img") return;
    ...
    useEffect(() => {
        if (!workspaceId) return;
        const handleExternalCommit = (e: any) => {
            const { newCommitPath } = e.detail;
            void loadWorkspaceGenerations({
                bumpCanvasRefreshKey: true,
                newCommitPath
            });
        ...
        window.addEventListener(`workspace-commit-${workspaceId}`, handleExternalCommit);
        return () => window.removeEventListener(`workspace-commit-${workspaceId}`, handleExternalCommit);
    }, [workspaceId, loadWorkspaceGenerations, setGenerationState]);
    ...
    useEffect(() => {
        if (!generation.loading && mode.inpaintMaskSnapshot && !generation.pendingRestart) {
            setModeState({
                inpaintMaskSnapshot: null,
                generationBounds: null,
            });
        }
    }, [generation.loading, mode.inpaintMaskSnapshot, setModeState, generation.pendingRestart]);
    ...
    // Auto-selection of first model removed to respect "None" state
    ...
    const handleCanvasImageUpload = async (imageSrc: string): Promise<void> => {
        ...
        // Auto-switch to img2img if we are in txt2img mode
        if (mode.generationMode === "txt2img") {
            handleGenerationModeChange("img2img");
        }
    ...
    const navigateTimeline = useCallback((direction: number) => {
        const preview = timeline.currentPreview;
        const queue = timeline.generationQueue;
        const committed = timeline.committedHistory;
        const discarded = timeline.discarded;

        const moveInList = (list: typeof queue, current: (typeof queue)[0] | null) => {
            if (list.length === 0) return;
            const idx = current ? list.findIndex((g) => g.genid === current.genid) : -1;
            const nextIdx = idx < 0 ? (direction > 0 ? 0 : list.length - 1) : (idx + direction + list.length) % list.length;
            handlePreviewSelect(list[nextIdx]);
        };

        if (preview?.status === "candidate") {
            moveInList(queue, preview);
            return;
        }
        if (preview?.status === "commit") {
            moveInList(committed, preview);
            return;
        }
        if (preview?.status === "reject") {
            moveInList(discarded, preview);
            return;
        }
        // Nothing selected (viewing canvas): down = first item of first list, up = last item of last list
        if (direction > 0) {
            if (queue.length) handlePreviewSelect(queue[0]);
            else if (committed.length) handlePreviewSelect(committed[0]);
            else if (discarded.length) handlePreviewSelect(discarded[0]);
        } else {
            if (discarded.length) handlePreviewSelect(discarded[discarded.length - 1]);
            else if (committed.length) handlePreviewSelect(committed[committed.length - 1]);
            else if (queue.length) handlePreviewSelect(queue[queue.length - 1]);
        }
    }, [timeline.currentPreview, timeline.generationQueue, timeline.committedHistory, timeline.discarded, handlePreviewSelect]);
    ...
    const getWorkspaceImagePath = (
        ...
        // If it's a direct Generation object
        ...
        // If it's a wrapper object from the UI
        if ("image" in sourceImage && sourceImage.image) {
            const workspaceInfo = parseWorkspaceImage(sourceImage.image);
            if (workspaceInfo) {
                // Return the path (which is category/genid/full.webp)
                return workspaceInfo.path;
            }
        }

        // Fallback for canvas if no image set yet
    ...
    const handleUpscale = async (upscaler: string, scaleFactor: number): Promise<void> => {
        ...
        try {
            ...
            const taskId = `task(extra-${Date.now()}-${Math.random().toString(36).substr(2, 9)})`;
    ...
    useKeyboardShortcuts({
        ...
        "arrowleft a": () => navigateCandidate(-1),
        "arrowright d": () => navigateCandidate(1),
        "arrowup w": () => navigateTimeline(-1),
        "arrowdown s": () => navigateTimeline(1),
        "backspace": () => {
        ...
        "ctrl+enter": () => {
            if (composerPrompt.trim()) void generateImage();
        },
        "ctrl+g": () => {
            if (composerPrompt.trim()) void generateImage();
        },
        "escape": () => {
            handlePreviewSelect(null);
        },
    ...
    const preview = timeline.currentPreview;
    const latestCommit = timeline.committedHistory[0];
    const isViewingTimelinePreview = Boolean(
        preview &&
        (preview.status === "candidate" ||
            preview.status === "reject" ||
            (preview.status === "commit" && latestCommit && preview.genid !== latestCommit.genid))
    );
        ...
        <CanvasSyncProvider>
            <div className={`flex-1 flex overflow-hidden ${isActive ? "" : "hidden"}`}>
                <Sidebar
                    ...
                    onEnterCanvasMode={() => setCanvasState({ maskCanvasFocused: true })}
                />
                ...
                {mode.generationMode === "inpaint" ? (
                    ...
                    <InpaintCanvas
                        ...
                        onImageUpload={mode.generationMode === "img2img" || mode.generationMode === "txt2img" ? handleCanvasImageUpload : undefined}
                ...
                {ui.settingsSidebarOpen && (
                    <SettingsSidebar
                        isOpen={ui.settingsSidebarOpen}
                        onClose={() => setUiState({ settingsSidebarOpen: false })}
                    />
                )}
