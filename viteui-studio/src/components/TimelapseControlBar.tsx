...
import { Play, ChevronRight, Check, Loader2, Folder, Plus } from 'lucide-react';
...
export const TimelapseControlBar = ({ workspaceId, onPreview, collapsed }: TimelapseControlBarProps) => {
    ...
    const [settings, setSettings] = useState({
        ...
        translate_speed: 6.0,
        calculate_diff: true,
        source_mask: "diiFmask",
        max_side: 2560,
    });

    const [maxSideInput, setMaxSideInput] = useState(settings.max_side.toString());

    const pollIntervalRef = useRef<number | null>(null);
    const handleCreate = async (inputTimelapse?: string) => {
        ...
        try {
            ...
            const response = await fetch(`${API_BASE_URL}/api/viteapi/timelapse/start`, {
                ...
                body: JSON.stringify({
                    ...
                    input_timelapse: inputTimelapse
                }),
            });
        ...
        <div className="flex flex-col gap-1 px-1">
            ...
            {isOpen && (
                <div className="p-2 rounded bg-studio-panel border border-studio-border space-y-2 animate-in slide-in-from-top-2 duration-200">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                        <div className="space-y-1">
                            <label className="text-[10px] text-studio-textSecondary uppercase tracking-wider block">Max Side (px)</label>
                            <input
                                type="text"
                                inputMode="numeric"
                                pattern="[0-9]*"
                                ...
                                onChange={(e) => {
                                    ...
                                    setSettings(prev => ({
                                        ...prev,
                                        max_side: raw === "" || !Number.isFinite(numeric) ? 0 : numeric,
                                    }));
                                }}
                                className="w-full bg-studio-surface border border-studio-border rounded px-2 py-1 text-xs text-studio-text focus:border-studio-accent outline-none"
                            />
                    ...
                    <ExistingTimelapsesList
                        workspaceId={workspaceId}
                        onPreview={onPreview}
                        isOpen={isOpen}
                        onExtend={handleCreate}
...
interface ExistingTimelapsesListProps {
    workspaceId: string;
...
const ExistingTimelapsesList = ({ workspaceId, onPreview, isOpen, onExtend, isGenerating }: ExistingTimelapsesListProps) => {
                ...
                {timelapses.map((t) => (
                    <div key={t.name} className="flex items-center justify-between gap-2 p-1 hover:bg-studio-surface rounded group">
                        ...
                        <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                            <button
                                onClick={() => onExtend(t.name)}
                                disabled={isGenerating}
                                className="p-1 hover:bg-studio-accent hover:text-studio-bg rounded text-studio-textSecondary transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                                title="Extend this timelapse with new commits"
                            >
                                <Plus size={10} />
                            </button>
                            <button
                                onClick={() =>
                                    onPreview(
`${API_BASE_URL}/api/viteapi/timelapse/file/${workspaceId}/${t.name}`
                                    )
                                }
